...
export function inputParent(
  ...
  return `grid grid-rows-1 border bdr-input-border-default-color box-border hover:bdr-input-border-hover-color bg-input-bg-color ${bg ? 'bg-application-theme' : ''}  rounded-input-border-radius ${error && validation ? 'bdr-input-border-error-color duration-200' : focus === true ? 'bdr-input-border-focus-color duration-200' : ' duration-200'}`;
}
...
export function icons(icon: boolean, iconPosition: string) {
  return `grid bg-light-gray justify-center items-center ${icon ? 'block' : 'hidden'} text-[#42526E] ${iconPosition === 'left' ? 'rounded-l-[4px]' : 'rounded-r-[4px]'}`;
}
...
export function outlineLabel(
  ...
  return `${error && validation ? 'text-color-input-error-font-color' : focus === true ? 'text-color-input-outline-label-font-color ' : 'text-color-input-label-font-color'} ${labelPosition === 'outline' ? `block absolute translate-y-[-50%] px-[4px] translate-x-2 ${value.length === 0 && !focus ? 'top-[50%] duration-300' : 'top-[0%] scale-75 duration-300 bg-input-outline-focus-label-bg-color'}` : 'hidden'}`;
}

export function input(icon: boolean, iconPosition: string, width?: number) {
  return `focus:outline-none h-full ${(width as number) < 100 ? 'pl-[6px] text-center' : 'px-3'} w-full text-color-input-value-font-color text-input-value-font-size font-input-value-font-weight tracking-wide	bg-input-bg-color ${icon ? (iconPosition === 'left' ? 'rounded-r-[5px]' : 'rounded-l-[5px]') : 'rounded-[5px]'}`;
}
