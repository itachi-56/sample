...
export function DropdownOptionParent() {
  return `absolute top-[5px] z-10 bg-model-background-color rounded-[4px] w-full overflow-hidden transition-all duration-300 p-[8px]`;
}

export function DropdownOption(options_icon_position: string) {
  return `rounded-[3px] text-text-color hover:bg-option-hover hover:pl-[3px] text-[14px] duration-300 ease-in-out cursor-pointer grid ${options_icon_position == 'left' ? 'grid-cols-[auto,auto,1fr]' : 'grid-cols-[auto,1fr,auto]'} items-center`;
}
...
export function MultiSelectOutlineLabel(
  ...
  return `text-md font-normal ${focus === true ? 'text-primary-color' : error && validation ? 'text-red' : 'text-readable-text-color'} ${label_position === 'outline' ? `block absolute translate-y-[-50%] px-1 ${value.length === 0 && !focus ? 'top-[50%] duration-300' : 'top-[0%] scale-75 duration-300 bg-background-color'}` : 'hidden'}`;
}
